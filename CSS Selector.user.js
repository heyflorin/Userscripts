// ==UserScript==
// @name         CSS Selector Picker (v1.21.2 scroll-sync overlays)
...
(() => {
  ...
  const addStyle = (css) => {
    ...
    try { if (typeof GM !== 'undefined' && typeof GM.addStyle === 'function') return GM.addStyle(css); } catch(_) {}
  ...
  async function copyText(text) {
    ...
    try { const ta = document.createElement('textarea');
      ...
      Object.assign(ta.style, {position:'fixed',top:'0',left:'0',width:'1px',height:'1px',opacity:'0',pointerEvents:'none',zIndex:2147483647});
      document.body.appendChild(ta); ta.focus(); ta.select();
      const ok = document.execCommand('copy'); document.body.removeChild(ta); if (ok) return true;
    } catch(_){}
    try { const div = document.createElement('div'); div.contentEditable = 'true'; div.innerText = text;
      Object.assign(div.style, {position:'fixed',top:'0',left:'0',opacity:'0',zIndex:2147483647});
  ...
  const esc  = (CSS && CSS.escape) ? (s)=>CSS.escape(s) : (s)=>String(s).replace(/[^a-zA-Z0-9_-]/g, ch => '\\' + ch);
  const PILL_MAX_CHARS = 100;
  const trim = (s, n=PILL_MAX_CHARS) => (s.length<=n ? s : (s.slice(0, n-1)+'…'));
  const looksUniqueToken = (v)=>{ if(!v) return false; const s=String(v);
  ...
  const addHideRule = (sel)=>{
    ...
    hiddenRules.add(sel);
    hideStyle.appendChild(document.createTextNode(`${sel}{display:none !important;}\n`));
  };
  ...
  // hover RAF
  let rafScheduledHover = false, pendingXY = null;

  // pinch-zoom suppression (picker ON)
  let zoomSuppressOn = false;

  // tap detection
  let pdX = 0, pdY = 0, pdTime = 0;
  const TAP_MS = 300, TAP_DIST = 10;

  const uiContains = (node) => !!(node && (node.closest('[data-picker-ui="1"]')));

  // ---------- selector builders ----------
  const avoidUtility = /^(col|row|grid|container|wrap|clearfix|hidden|show|sr-only|visually-hidden|flex|items-|justify-|text-|mt-|mb-|ml-|mr-|p-|m-|w-|h-)/;

  function stableAttrNoId(el) {
    const attrs = ['data-testid','data-test','data-qa','aria-label','role','name'];
  ...
  function stableClasses(el, max=3) {
    const cl = Array.from(el.classList || []).filter(c => !avoidUtility.test(c) && !looksUniqueToken(c));
    if (!cl.length) return [];
    const withCounts = cl.map(name => ({name, count: document.getElementsByClassName(name).length || 0, len: name.length}))
  ...
  function bestClassGeneric(el) {
    ...
    const scored = classes.map(name => ({ name, count: document.getElementsByClassName(name).length || 0, len: name.length }))
                          .sort((a,b) => (a.count - b.count) || (b.len - a.len));
    for (const s of scored) { if (s.count <= 1) continue; if (avoidUtility.test(s.name)) continue; if (looksUniqueToken(s.name)) continue; return s; }
    for (const s of scored) { if (avoidUtility.test(s.name)) continue; if (looksUniqueToken(s.name)) continue; return s; }
    return null;
  }
  ...
  function buildGeneric(el) {
    ...
    const childSel = bc ? `.${esc(bc.name)}` : (attrSelectorGeneric(el) || el.tagName.toLowerCase());
    const p = el.parentElement;
    if (!p || !(p instanceof Element)) return childSel;
    const pab = bestClassGeneric(p);
    const paa = attrSelectorGeneric(p);
    const parentSel = pab ? `.${esc(pab.name)}` : (paa || p.tagName.toLowerCase());
  ...
  function anchorNoId(el) {
    ...
    if (cls.length) return `${tag}${cls.map(c=>'.'+esc(c)).join('')}${attr}`;
    if (attr) return `${tag}${attr}`;
    return tag;
  }
  const nthOfTypeIndex = (el)=>{ const p=el.parentElement; if(!p) return 1; const sib=[...p.children].filter(n=>n.tagName===el.tagName); return Math.max(1, sib.indexOf(el)+1); };
  const nthChildIndex  = (el)=>{ const p=el.parentElement; if(!p) return 1; const all=[...p.children]; return Math.max(1, all.indexOf(el)+1); };
  function buildSpecific(el) {
    if (!(el instanceof Element)) return '*';
    const MAX_ANC = 6;
    const ancEls = []; let cur = el.parentElement;
    while (cur && ancEls.length < MAX_ANC) { ancEls.unshift(cur); cur = cur.parentElement; }
    const ancAnchors = ancEls.map(a => anchorNoId(a));
    const ancSuffix  = ancEls.map(_ => '');
    const selfAnchorBase = anchorNoId(el);
    const prefixString = () => ancAnchors.map((a,i)=> a + ancSuffix[i]).join(' > ');
    const candidateWithSelf = (ixKind='type') => {
      const selfNth = (ixKind==='type') ? `:nth-of-type(${nthOfTypeIndex(el)})` : `:nth-child(${nthChildIndex(el)})`;
      const self = selfAnchorBase + selfNth;
      const pref = prefixString();
      return pref ? `${pref} > ${self}` : self;
    };
    let candidate = candidateWithSelf('type');
    let list = []; try { list = Array.from(document.querySelectorAll(candidate)); } catch(_){}
    if (list.length !== 1) {
      candidate = candidateWithSelf('child');
      try { list = Array.from(document.querySelectorAll(candidate)); } catch(_){}
    }
    if (list.length !== 1 && ancEls.length) {
      for (let i = ancEls.length - 1; i >= 0 && list.length !== 1; i--) {
        const aEl = ancEls[i];
        ancSuffix[i] = `:nth-of-type(${nthOfTypeIndex(aEl)})`;
        candidate = candidateWithSelf('type'); try { list = Array.from(document.querySelectorAll(candidate)); } catch(_){}
    ...
    while (list.length !== 1 && cur && cur instanceof Element) {
      ancEls.unshift(cur); ancAnchors.unshift(anchorNoId(cur)); ancSuffix.unshift(`:nth-of-type(${nthOfTypeIndex(cur)})`);
      candidate = candidateWithSelf('type'); try { list = Array.from(document.querySelectorAll(candidate)); } catch(_){}
      cur = cur.parentElement; if (ancEls.length > 12) break;
    }
    return candidate;
  }

  // ---------- overlays ----------
  const isPickerNode = (n)=> !!(n && n.closest && n.closest('[data-picker-ui="1"]'));
  function clearMatches(){ while (matchesLayer.firstChild) matchesLayer.removeChild(matchesLayer.firstChild); }
  function boxForRect(rect, cls='picker-match-box'){
    const d=document.createElement('div'); d.className=cls; d.setAttribute('data-picker-ui','1');
    d.style.left=rect.left+'px'; d.style.top=rect.top+'px'; d.style.width=rect.width+'px'; d.style.height=rect.height+'px';
    return d;
  }
  function drawHighlightsFor(el, sel){
    clearMatches();
    let matches=[]; try { matches = Array.from(document.querySelectorAll(sel)); } catch(_){}
    const LIMIT=1200; let count=0;
    for (const m of matches){
      if (count>=LIMIT) break;
      if (!(m instanceof Element)) continue;
      if (m === el) continue;
      if (isPickerNode(m)) continue;
      const r=m.getBoundingClientRect(); if (!r.width||!r.height) continue;
      const box = boxForRect(r,'picker-match-box');
      if (locked) box.classList.add('locked');
      matchesLayer.appendChild(box); count++;
    }
    const r0=el.getBoundingClientRect();
    if (!r0.width||!r0.height){ hoverBox.style.display='none'; }
    else {
      hoverBox.style.display='block';
      hoverBox.style.left=r0.left+'px'; hoverBox.style.top=r0.top+'px';
      hoverBox.style.width=r0.width+'px'; hoverBox.style.height=r0.height+'px';
  ...
  function computeSelectorsFor(el){ selectorGeneric=buildGeneric(el); selectorSpecific=buildSpecific(el); }
  function updateResultsUI(targetEl){
    const gEl = results.querySelector('.picker-row[data-kind="generic"]  [data-role="label"]');
    const sEl = results.querySelector('.picker-row[data-kind="specific"] [data-role="label"]');
    if (!targetEl){ results.style.display='none'; return; }
  ...
  function updateVisuals(){
    ...
    if(!pickMode||!tgt){ results.style.display = pickMode?'flex':'none'; hoverBox.style.display='none'; clearMatches(); return; }
    drawHighlightsFor(tgt, selectorGeneric);
    updateResultsUI(tgt);
  }
  function setLockedTarget(el){
    if(!(el instanceof Element) || isPickerNode(el)) return false;
    locked = true; lockedTarget = el;
    computeSelectorsFor(el); updateVisuals(); return true;
  }
  ...
  function queueFromXY(x,y){
    ...
    if (!rafScheduledHover){
      ...
      requestAnimationFrame(()=>{
        ...
        const p = pendingXY; pendingXY = null;
        ...
        if(!(target instanceof Element)){
          hoveredTarget=null; selectorGeneric=''; selectorSpecific='';
          results.style.display='none'; hoverBox.style.display='none'; hoverBox.classList.remove('locked'); clearMatches(); return;
        }
        hoveredTarget=target; computeSelectorsFor(target); updateVisuals();
      });
    }
  }
  ...
  document.addEventListener('pointerdown',(e)=>{
    if(!pickMode) return;
    pdX=e.clientX; pdY=e.clientY; pdTime=performance.now();
    if(!uiContains(e.target)) queueFromXY(e.clientX, e.clientY);
  },{capture:true,passive:true});

  document.addEventListener('pointermove',(e)=>{ if(!pickMode) return; if(!locked) queueFromXY(e.clientX, e.clientY); },{capture:true,passive:true});

  document.addEventListener('pointerup',(e)=>{
    if(!pickMode) return;
    if(uiContains(e.target)) return;
    const dt=performance.now()-pdTime; const d=Math.hypot((e.clientX-pdX)||0,(e.clientY-pdY)||0);
    if(dt<=TAP_MS && d<=TAP_DIST){
      if(!locked){ if(hoveredTarget instanceof Element){ locked=true; lockedTarget=hoveredTarget; } }
      else { locked=false; lockedTarget=null; hoverBox.classList.remove('locked'); }
      updateVisuals();
    }
  },{capture:true,passive:true});
  ...
  document.addEventListener('touchstart',(e)=>{
    ...
    if(!pickMode) return;
    const t=e.touches[0]; if(t){ lastTouchX=t.clientX; lastTouchY=t.clientY; }
  },{capture:true});
  document.addEventListener('touchmove',(e)=>{
    if(!pickMode) return;
    if (e.touches.length === 1 && !locked){
      const t=e.touches[0];
      if (t){ queueFromXY(t.clientX, t.clientY); }
  ...
  document.addEventListener('touchend',(e)=>{ activeTouchCount=e.touches.length; },{capture:true});
  document.addEventListener('touchcancel',()=>{ activeTouchCount=0; },{capture:true});
  ...
  for (const type of ['click','mousedown','mouseup','pointerup','dblclick','submit']) {
    document.addEventListener(type, cancelPageAction, true);
  }

  // ---------- traversal / hide actions ----------
  function validElement(el){ return el && el instanceof Element && !isPickerNode(el); }
  function firstValidChild(el){ let c=el && el.firstElementChild; while (c && isPickerNode(c)) c=c.nextElementSibling; return c; }
  function prevValidSibling(el){ let p=el && el.previousElementSibling; while (p && isPickerNode(p)) p=p.previousElementSibling; return p; }
  function nextValidSibling(el){ let n=el && el.nextElementSibling; while (n && isPickerNode(n)) n=n.nextElementSibling; return n; }

  function traverse(kind, action){
    let base = currentTarget();
    if (!base) return;
    if (!locked) { if (!setLockedTarget(base)) return; base = lockedTarget; }

    let target = null;
    if (kind === 'specific'){ // siblings
      if (action === 'prev') target = prevValidSibling(base);
      else if (action === 'next') target = nextValidSibling(base);
    } else { // generic: parents / children
      if (action === 'parent') target = validElement(base.parentElement) ? base.parentElement : null;
      else if (action === 'child') target = firstValidChild(base);
    }

    const pill = results.querySelector(`.picker-row[data-kind="${kind}"] [data-role="label"]`);
    if (validElement(target)) { setLockedTarget(target); }
    else {
      const msg = (action==='parent')?'No parent':(action==='child')?'No child':(action==='prev')?'No prev sibling':'No next sibling';
      const restore = (kind==='generic') ? selectorGeneric : selectorSpecific;
      if (pill) { const r = restore; pill.textContent = msg; setTimeout(()=>{ pill.textContent = trim(r,PILL_MAX_CHARS); }, 900); }
    }
  }
  ...
  const copyFromPill = async (pillEl)=>{
    ...
    setTimeout(()=>{ pillEl.textContent=trim(restore,PILL_MAX_CHARS); }, 900);
  };
  results.addEventListener('click', (e)=>{
    const pillEl = e.target.closest('.selector-pill'); if(!pillEl) return;
  ...
  results.addEventListener('click', (e)=>{
    ...
    e.preventDefault(); e.stopPropagation();
    const row = btn.closest('.picker-row'); const kind = row?.getAttribute('data-kind');
    const action = btn.getAttribute('data-action');
    if (action === 'hide'){
      const tgt = currentTarget(); if (!tgt) return;
      const sel = (kind === 'specific') ? (selectorSpecific || buildSpecific(tgt))
                                        : (selectorGeneric  || buildGeneric(tgt));
      addHideRule(sel);
      const pill = row.querySelector('.selector-pill'); if(pill){ const r = sel; pill.textContent='Hidden'; setTimeout(()=>{ pill.textContent=trim(r,PILL_MAX_CHARS); }, 900); }
      return;
    }
  ...
  // ---------- snapping logic ----------
  const SNAP_EDGE_PX=12, SIMILAR_SIDE_PX=16, SIMILAR_AREA_RATIO=1.35;
  const rectLike=(a,b)=>{
    const aw=a.width,ah=a.height,bw=b.width,bh=b.height; if(!aw||!ah||!bw||!bh) return false;
    const similarSides = Math.abs(a.left-b.left)<=SIMILAR_SIDE_PX && Math.abs(a.top-b.top)<=SIMILAR_SIDE_PX &&
                         Math.abs(a.right-b.right)<=SIMILAR_SIDE_PX && Math.abs(a.bottom-b.bottom)<=SIMILAR_SIDE_PX;
    const arA=aw*ah, arB=bw*bh, ratio=arA>arB?arA/arB:arB/arA; return similarSides || ratio<=SIMILAR_AREA_RATIO;
  };
  const distToRectEdge=(r,x,y)=>Math.min(Math.abs(x-r.left),Math.abs(r.right-x),Math.abs(y-r.top),Math.abs(r.bottom-y));
  const pointInRect=(r,x,y)=>x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom;
  function nearestBoundaryChild(el,x,y,parentRect){
    const n=Math.min(el.children.length,50); let best=null, bestEdge=Infinity;
    for(let i=0;i<n;i++){ const c=el.children[i]; if(!(c instanceof Element)) continue;
      const cr=c.getBoundingClientRect(); if(!cr.width||!cr.height) continue; if(!pointInRect(cr,x,y)) continue;
      if(!(rectLike(cr, el.getBoundingClientRect())||(parentRect&&rectLike(cr,parentRect)))) continue;
      const d=distToRectEdge(cr,x,y); if(d<=SNAP_EDGE_PX && d<bestEdge){ best=c; bestEdge=d; }
    } return best;
  }
  function snapCandidate(el,x,y){
    if(!(el instanceof Element)) return el;
    const r=el.getBoundingClientRect(), p=el.parentElement; let parentPick=null, dParent=Infinity;
    if(p && p instanceof Element){
      const pr=p.getBoundingClientRect(); const dEdge=distToRectEdge(r,x,y);
      if(dEdge<=SNAP_EDGE_PX && rectLike(pr,r)){
        parentPick=p; dParent=Math.min(Math.abs(x-pr.left),Math.abs(pr.right-x),Math.abs(y-pr.top),Math.abs(pr.bottom-y));
      }
    }
    const childPick=nearestBoundaryChild(el,x,y, p?p.getBoundingClientRect():null);
    if(parentPick && childPick){ const cr=childPick.getBoundingClientRect(); const dChild=distToRectEdge(cr,x,y); return dParent<=dChild?parentPick:childPick; }
    return parentPick || childPick || el;
  }

  // ---------- block pinch-zoom while picker ON ----------
  const preventZoomGesture = (e)=>{ if (pickMode) { e.preventDefault(); } };
  const preventCtrlWheel = (e)=>{ if (pickMode && (e.ctrlKey || e.metaKey)) { e.preventDefault(); } };
  ...
  function disableZoomSuppression(){
    if (!zoomSuppressOn) return;
    document.removeEventListener('gesturestart', preventZoomGesture, {capture:true});
    document.removeEventListener('gesturechange', preventZoomGesture, {capture:true});
    document.removeEventListener('gestureend', preventZoomGesture, {capture:true});
    document.removeEventListener('wheel', preventCtrlWheel, {capture:true});
    document.removeEventListener('dblclick', preventZoomGesture, {capture:true});
  ...
  window.addEventListener('resize', () => { if (pickMode && currentTarget()) updateVisuals(); }, { capture: true, passive: true });

  // ---------- mode toggles (grabber + swipe zone + hotkey) ----------
  function applyZoomPolicy(){ if (pickMode) enableZoomSuppression(); else disableZoomSuppression(); }
  ...
  function setPickMode(on){
    ...
    hoveredTarget = null; selectorGeneric=''; selectorSpecific='';
    results.style.display='none'; hoverBox.style.display='none'; clearMatches();
    applyZoomPolicy();
  }

  // Tap the grabber to toggle
  grabber.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); setPickMode(!pickMode); }, {passive:false});

  // Swipe logic (open when picker off; close when on)
  function installSwipeOpenClose(el){
    let sx=0, sy=0, down=false;
    const THRESH=28;
    el.addEventListener('pointerdown', (e)=>{ if (el.classList.contains('disabled')) return; sx=e.clientX; sy=e.clientY; down=true; }, {passive:true});
    el.addEventListener('pointermove', (e)=>{
      if (!down || el.classList.contains('disabled')) return;
      const dx=(e.clientX - sx) || 0, dy=(e.clientY - sy) || 0;
    ...
    el.addEventListener('pointerup',   ()=>{ down=false; }, {passive:true});
  ...
  document.addEventListener('keydown',(e)=>{
    ...
    if(typing) return;
    if(e.metaKey&&e.shiftKey&&(e.key==='C'||e.key==='c')){ e.preventDefault(); e.stopPropagation(); setPickMode(!pickMode); }
  },true);
